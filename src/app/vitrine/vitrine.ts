...
export class Vitrine {
  ...
  adicionarCesta(produto: Produto) {
    const preco = produto.promo > 0 ? produto.promo : produto.valor;
    ...
    if (cestaJson == null) {
      ...
      itens = JSON.parse(cestaJson);
    }

    let encontrado = false;
    for (let obj of itens) {
      if (obj.produto.codigo == produto.codigo) {
        obj.quantidade += 1;
        obj.valorTotal = obj.quantidade * preco;
        encontrado = true;
        break;
      }
    }
